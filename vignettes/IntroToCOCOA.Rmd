...
COCOA uses a database of region sets to annotate inter-sample DNA methylation variation in your data. In this vignette, we will see how COCOA can find meaningful sources of DNA methylation variation in breast cancer patients. 
...
As an easy way to visualize how the region sets are ranked for each PC, we can use `rsScoreHeatmap`:
...
signalAlongAxis(genomicSignal=brcaATACData1,
                ...
                name = "Signal in ATAC-seq regions",
                show_row_names=FALSE,
                show_column_names=FALSE)
```

Looking at the raw data confirms that DNA methylation is in fact varying along the PC axis. It appears that some but not all CpGs vary greatly along the PC axis. The CpGs that show high variation along the PC axis are the ones that contribute to the ESR1 region set being ranked highly in our analysis.
...
### Making a "meta-region" loading profile {#mrProfile}
