...
For a more in-depth description of the method, see the "[Basic workflow]" and "[Method details]" sections of this vignette. 
...
# COCOA for DNA methylation data

COCOA uses a database of region sets to annotate inter-sample DNA methylation variation in your data. In this vignette, we will see how COCOA can find meaningful sources of DNA methylation variation in breast cancer patients. We will use data from The Cancer Genome Atlas: 450k DNA methylation microarrays for 657 breast cancer patients (TCGA-BRCA, https://portal.gdc.cancer.gov/).

First, we will show how to use COCOA to find region sets where DNA methylation variation is associated with variation in some phenotype of interest (the target variable) ([Supervised COCOA]). Then, we will show how to find region sets that are associated with the principle components of PCA of our DNA methylation data ([Unsupervised COCOA]). 
...
```{r, eval=TRUE, message=FALSE, warning=FALSE}
targetVar$ER_Status <- scale(as.numeric(as.factor(targetVar$ER_Status)), 
                               center=TRUE, scale=FALSE)
methylCor <- cor(t(brcaMethylData1), targetVar$ER_Status, 
...
Now we have a score for each region set that represents how much epigenetic variation in that region set is associated with the phenotype of interest, ER status. While this vignette is limited by having four samples (only one of which is ER-), if more samples were included, it would become clear that the ER and GATA3 region sets are much more highly associated with ER status than the NRF1 and ATF3 region sets.
...
We now have a list where each item is a result of `aggregateSignalGRList`. We have `nPerm` list items in permRSScores. We'll reformat it so that we have a list where each item is a null distribution for a single region set: 
...
Fitting a gamma distribution to only 5 values does not really make sense so that could be one reason the p-values for ER and GATA3, which have been associated with DNA methylation) are not significant. Also, with only 4 samples, shuffling the sample phenotypes is not as effective since there is a limited number of unique permutations.

We can also use the null distribution to directly calculate empirical p-values:
...
The empirical p-values are limited by the number of permutations done so the gamma distribution approximation is recommended to allow the possibility of lower p-values.


```{r, eval=FALSE}
PCsToAnnotate <- c("PC1", "PC2")
targetVar = data.frame(PC1=aPCA$x[, "PC1"], PC2=aPCA$x[, "PC2"])
targetVar <- as.data.frame(scale(targetVar, 
                               center=TRUE, scale=FALSE))
...
This section of the vignette begins after principal component analysis (PCA) has been performed on the DNA methylation data. As you can see below, PC1 and PC4 cluster patients based on estrogen receptor status which is an important prognostic marker for breast cancer: 
...
There are three main inputs for unsupervised COCOA:
...
```{r, eval=TRUE, message=FALSE, warning=FALSE}
mPCA <- prcomp(t(brcaMethylData1))

PCsToAnnotate <- paste0("PC", 1:4)
targetVar <- mPCA$x[, PCsToAnnotate]
...
# cor() will return NA, so manually set the correlation to 0 for these CpGs
methylCor[is.na(methylCor)] <- 0
colnames(methylCor) <- PCsToAnnotate
```
...
# COCOA for chromatin accessibility data (ATAC-seq)
...
regionSetScores <- aggregateSignalGRList(signal=mPCA$rotation, 
