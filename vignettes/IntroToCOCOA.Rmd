...
```{r, eval=TRUE, message=FALSE, warning=FALSE}
GRList <- GRangesList(esr1_chr1=esr1_chr1, gata3_chr1=gata3_chr1, atf3_chr1=atf3_chr1, nrf1_chr1=nrf1_chr1)
...
rsScores <- aggregateSignalGRList(signal=methylCor,
                     ...
                     absVal=TRUE)
rsScores$regionSetName <- names(GRList)
rsScores
```
...
realRSScores <- runCOCOA(genomicSignal=brcaMethylData1,
                         ...
                         variationMetric="cor")
realRSScores$regionSetName <- regionSetNames
realRSScores
```

### Run COCOA permutations to get p-values

Now we have the real COCOA score and we can run the `runCOCOAPerm` function to get p-values. This is a convenience function that runs multiple steps of the permutation process together: it runs COCOA permutations, converts these to null distributions, gets the empirical p value (which is limited by the number of permutations), gets z scores, and fits a gamma distribution to each null distribution to estimate p values (not limited by the number of permutations). We will be running five permutations for the vignette but many more should be done for a real analysis (probably at least a few hundred). We need to set the seed for reproducible results since we are doing random shuffling of the sample phenotypes.
...
To reduce the number of permutations (and the run time for COCOA), we will approximate each null distribution with a gamma distribution [(Winkler et al., 2016)](#refs). After fitting the gamma distribution to each null distribution, we can used the gamma distributions to get p-values. We fit the gamma distribution and get the p-value with `getGammaPVal`. Take a look at the COCOA paper for more information on the best use of the gamma approximation (it may overestimate significance of low p-values).
...
```{r, eval=TRUE, message=FALSE, warning=FALSE}
# prepare data with region names
GRList <- GRangesList(esr1_chr1=esr1_chr1, gata3_chr1=gata3_chr1, nrf1_chr1=nrf1_chr1, atf3_chr1=atf3_chr1) 
```
...
regionSetScores <- aggregateSignalGRList(signal=methylCor, 
                            ...
                            scoringMetric="default")
regionSetScores$regionSetName <- names(GRList)
regionSetScores
```
...
# Change brcaATACCoord1 to another format (only start position)
start_positions <- start(brcaATACCoord1)
brcaATACCoord1 <- GRanges(seqnames=seqnames(brcaATACCoord1), 
                          ranges=IRanges(start=start_positions, width=1),
                          strand=strand(brcaATACCoord1))
```
...
correctSampleOrder=1:nrow(targetVarDF)
realRSScores <- runCOCOA(genomicSignal=brcaATACData1,
                         signalCoord=brcaATACCoord1,
                         GRList=GRList,
                         signalCol=myPhen,
                         targetVar=targetVarDF,
...
permResults <- runCOCOAPerm(genomicSignal=brcaATACData1,
                            signalCoord=brcaATACCoord1,
                            GRList= GRList,
                            rsScores=realRSScores,
                            targetVar=targetVarDF,
