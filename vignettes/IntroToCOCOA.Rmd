...
Then we will do PCA on our breast cancer data.
...
COCOA will still work even if there are not distinct clusters of samples in your PCA plot and if you do not have known groups of samples. In this vignette, we will look at principal components 1-4 but this choice depends on the context of your analysis. 
...
As you can see below, PC1 and PC4 cluster patients based on estrogen receptor status which is an important prognostic marker for breast cancer: 
...
Based on this, it is not surprising that the variation captured by these PCs would be associated with estrogen receptor (consistent with interpatient variation in estrogen receptor status). Outside of this vignette, COCOA was run with a region set database of over 2000 region sets. To keep things simple for this vignette, we only used two of the highest scoring region sets and two of the lowest scoring region sets from that analysis to demonstrate how COCOA works.
...
As mentioned before, this vignette is showing two of the top scoring and bottom scoring region sets from a previous COCOA analysis with over 2200 region sets.
